...
void fill_unfolding_responsematrix_withsets(TH2D* hist_response_matrix[11], double fillvar1, double fillvar2, double weight, int exlude_set)
{
  hist_response_matrix[0]->Fill(fillvar1,fillvar2,weight);
  for(int ih = 1; ih< 11; ih++)
  {
    if(ih != exlude_set)
      hist_response_matrix[ih]->Fill(fillvar1,fillvar2,weight);
  }
}

void fill_unfolding_variable_withsets(TH1D* hist_variable[11], double fillvar1, double weight, int exlude_set)
{
  hist_variable[0]->Fill(fillvar1,weight);
  for(int ih = 1; ih< 11; ih++)
  {
    if(ih != exlude_set)
      hist_variable[ih]->Fill(fillvar1,weight);
  }
}
